...
export default function ImageZoomPreview({ images, index, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Prev */}
      <button onClick={prev} className="absolute left-4 z-50 rounded-full bg-white/90 p-2">
        <ChevronLeft />
      </button>

      {/* Next */}
      <button onClick={next} className="absolute right-4 z-50 rounded-full bg-white/90 p-2">
        <ChevronRight />
      </button>
